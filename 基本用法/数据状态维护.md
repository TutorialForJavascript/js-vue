...
const getters = {
    ...
    first5heros: (state) => {
        ...
        if (state.heros.length > 0) {
            ...
            return heros_copy.sort((herof, herol) => herol - herof).slice(0, 5)
        } else {
            return []
        }
...
由于这几个映射的用法基本一样,只是`mapState`由于我们是放在`computed`中所以需要将其放到函数里.因此我们就不一一举例,这边我们以`mapGetters`为`computed`添加字段为例:
